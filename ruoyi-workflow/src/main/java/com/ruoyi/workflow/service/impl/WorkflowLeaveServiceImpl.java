...
import com.ruoyi.workflow.mapper.WorkflowLeaveMapper;
import com.ruoyi.workflow.domain.WorkflowLeave;
import com.ruoyi.workflow.service.IWorkflowLeaveService;
...
public class WorkflowLeaveServiceImpl implements IWorkflowLeaveService {
  ...
  private static final Logger log = LoggerFactory.getLogger(WorkflowLeaveServiceImpl.class);
  @Autowired
  ...
  public int insertWorkflowLeave(WorkflowLeave workflowLeave) {
    ...
    String deptLeaders = StringUtils.join(sysUserService.selectUserNameByPostCodeAndDeptId("se", SecurityUtils.getLoginUser().getUser().getDeptId()), ",");
    log.info("发起流程,部门领导: {}",deptLeaders);
    String hrs = StringUtils.join(sysUserService.selectUserNameByPostCode("hr"), ",");
    log.info("发起流程,hr: {}",hrs);
    ProcessInstance processInstance = processRuntime.start(ProcessPayloadBuilder
            .start()
            .withProcessDefinitionKey("leave")
            .withName(workflowLeave.getTitle())
            .withBusinessKey(id)
            .withVariable("deptLeader", deptLeaders)
            .withVariable("admin", "admin")
            .withVariable("hr", hrs)
            .withVariable("leaveDays", ChronoUnit.DAYS.between(workflowLeave.getLeaveStartTime().toInstant(), workflowLeave.getLeaveEndTime().toInstant()))
            .build());

    //模拟知会功能, 知会到zhugeliang
//    CreateTaskPayload build = new CreateTaskPayloadBuilder().withName(processInstance.getId())
//            .withDescription("模拟知会任务")
//            .withAssignee("zhugeliang")
//            .withFormKey("@@zhihui")
//            .withDueDate(new Date()).build();
//    taskRuntime.create(build);
//    log.info("模拟知会通知到zhugeliang");
