...
import com.ruoyi.activiti.manage.ProcessInstanceManager;
import com.ruoyi.activiti.service.image.ProcessInstanceService;
...
public class ProcessInstanceServiceImpl implements ProcessInstanceService {
  ...
  /**
   * Desc:无论任何情况，清除所有的流程实例，实质上还是一个一个删除 当本身没有流程实例的时候使用此方法也不会报错
   */
  @Override
  ...
  /**
   * Desc:通过流程定义name获取有关于其所有的流程实例 (想要同时出现不同版本但是name相同的流程定义是不可能的，如果想要修改流程定义，必须使其现有的所有流程实例都结束或废弃)
   *
   * @param processDefinitionName 流程定义name
  ...
  /**
   * Desc:根据流程定义name和业务标识进行寻找流程实例 (在存储的时候使用uuid当做businessKey，最大可能性的避免重复)
   *
   * @param processDefinitionName 流程定义name
