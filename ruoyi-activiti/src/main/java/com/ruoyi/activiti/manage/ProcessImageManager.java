...
import com.ruoyi.activiti.service.image.diagram.CustomProcessDiagramGenerator;
...
public class ProcessImageManager {
  ...
  private Logger log = LoggerFactory.getLogger(ProcessImageManager.class);
  ...
  public InputStream getFlowImgByProcInstId(String procInstId) throws Exception {
    ...
    try {
      ...
      List<HistoricActivityInstance> historicActivityInstanceList = processHistoryManager.getHistoryActivitiByStartTimeAsc(procInstId);
...
//      List<String> highLightedFlowsIds = getHighLightedFlowsByIncomingFlows(bpmnModel, historicActivityInstanceList);
      List<String> highLightedFlowsIds = processTraceService.getHighLine(bpmnModel, procInstId);
...
//                log.info("【完成】-匹配当前节点-ActivityId=【{}】需要高亮显示的流出线", currentActivityId);
  ...
  /**
   * 解决getRunningActivityFlowsIds对于并行网关不适用的问题 提供者：孙之峰（CSDN)
   */
