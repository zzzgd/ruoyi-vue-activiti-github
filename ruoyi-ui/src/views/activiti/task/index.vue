<template>
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open" v-if="open" width="500px" append-to-body>
      ...
      <customercomplaint :businessKey="businessKey" v-if="'ticket_consumer_complaint'==definitionKey"/>
      ...
      <el-form :model="form" ref="form" label-width="100px" class="demo-dynamic">
        ...
        >
          ...
          <el-select v-model="domain.controlValue" v-if="'select'==domain.controlType">
            <el-option
                v-for="dict in domain.controlSelect"
                :key="dict.dictValue"
          ...
          <el-input type="text" v-model="domain.controlValue" v-if="'text'==domain.controlType"
          ></el-input>
        </el-form-item>
        <el-form-item
            v-if="form.formData.length ===0"
        >
          是否审核通过?
        </el-form-item>
      </el-form>
...
import customercomplaint from "@/views/workflow/customercomplaint/customercomplaintHistoryForm";

export default {
  name: "Leave",
  components: {leaveHistoryForm, customercomplaint},
  data() {
    return {
      id: '',
      definitionKey: '',
  ...
  methods: {
    ...
    async syncGetDict(type) {
      let value = null
      await this.getDicts(type).then(response => {
    ...
    examineAndApprove(row) {
      ...
      formDataShow(row.id).then(async response => {
        ...
        console.log('formDataShow:', datas)
        let formData = []
        for (let i = 0; i < datas.length; i++) {
          ...
          switch (strings[1]) {
            ...
            case 'select':
              controldefault = strings[4]
              let data = await this.syncGetDict(controldefault)
              controlSelect = data
              break;
              // default:
          }
