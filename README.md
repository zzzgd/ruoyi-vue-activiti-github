## 一. 介绍
基于[https://gitee.com/smell2/ruoyi-vue-activiti](https://gitee.com/smell2/ruoyi-vue-activiti) 项目改造

- 增加dockerfile, 打包docker部署前端代码
- 解决原项目的几个bug, 目前能正常启动跑起来, 其中包括edge浏览器无法查看审批进度的问题
...
### 2.1 前端
#### 2.1.1 docker
- 根据dockerfile, 构建镜像, 然后docker运行前端
#### 2.1.2 yarn
- 在node环境中, 安装yarn, 直接npm好像有点问题, 一直不成功.
- `yarn`解析依赖
- `yarn dev` 启动项目

### 2.2 后端
- 修改ruoyi-admin中resources中的配置文件, mysql配置, 但是不要最好不要改mysql链接后面的一串参数
- 执行sql文件
- 启动, 自动生成activiti表
- 整体启动后, 部署leave.bpmn流程
